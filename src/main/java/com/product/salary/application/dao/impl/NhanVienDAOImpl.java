...
public class NhanVienDAOImpl extends AbstractDAO implements NhanVienDAO, Serializable {
	...
	public NhanVien themNhanVien(NhanVien nhanVien) {
		...
		if (connect != null) {
			try {
				StringBuilder query = new StringBuilder(
						"INSERT INTO NhanVien (MaNhanVien, HoTen, Email, DiaChi, GioiTinh, MaChucVu, Cccd, SoDienThoai, NgaySinh, MaPhongBan, NgayVaoLam, LuongCoSo, HeSoLuong, TroCap, TrangThai, HinhAnh, MaTrinhDo)");
				query.append(" VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

				state = connect.prepareStatement(query.toString());

				state.setString(1, nhanVien.getMaNhanVien());
				state.setString(2, nhanVien.getHoTen());
				state.setString(3, nhanVien.getEmail());
				state.setString(4, nhanVien.getDiaChi());
				state.setInt(5, nhanVien.getGioiTinh());
				state.setString(6, nhanVien.getChucVu().getMaChucVu());
				state.setString(7, nhanVien.getCccd());
				state.setString(8, nhanVien.getSoDienThoai());
				state.setDate(9, Date.valueOf(nhanVien.getNgaySinh()));
				state.setString(10, nhanVien.getPhongBan().getMaPhongBan());
				state.setDate(11, Date.valueOf(nhanVien.getNgayVaoLam()));
				state.setDouble(12, nhanVien.getLuongCoSo());
				state.setDouble(13, nhanVien.getHeSoLuong());
				state.setDouble(14, nhanVien.getTroCap());
				state.setBoolean(15, nhanVien.isTrangThai());
				state.setBytes(16, nhanVien.getHinhAnh());
				state.setString(17, nhanVien.getTrinhDo().getMaTrinhDo());
	...
	public String layMaNhanVienCuoiCungCuaNam(int nam) {
		...
		if (connect != null) {
			try {
				String query = "SELECT TOP 1 MaNhanVien FROM NhanVien WHERE YEAR(NgayVaoLam) = " + nam
